create table if not exists public.booking_capacity_overrides (
  ...
  extra_bookings smallint not null default 1 check (extra_bookings between 1 and 3),
  reason text,
...
begin
  if new.status in ('pending', 'confirmed')
    ...
    and (select count(*) from public.bookings where booking_date = new.booking_date and status in ('pending', 'confirmed') and id <> new.id)
      >= coalesce((select 1 + extra_bookings from public.booking_capacity_overrides where booking_date = new.booking_date), 1)
  then
